...
def flatten_peak(peak):
    return peak.chrom, peak.start, peak.stop, peak.name, peak.strand, peak.attrs["gene"], peak.attrs["genesymbol"], peak.attrs["gtype"], peak.attrs["tss"], peak.attrs["atg"], peak.attrs["annotation"], peak.attrs["function"], peak.attrs['fold'], peak.attrs["topcoverage"], peak.attrs["other_topcoverage"], peak.attrs["cg"]
...
header_attrs =  ['geneID', 'genesymbol', 'gtype', 'tss', 'atg', 'annotation', 'function', 'fold_change', 'topcoverage', 'other_topcoverage', 'cg']
...
with doc:
    ...
    input(type="number", id="inp4", onkeyup='my_filter_greater(13, "inp4", "myTable")', placeholder="Filter fold change greater than..")
...
with open(os.path.join(args.outdir, name + ".html" ), 'w') as f:
    #print(doc.render())
    f.write(doc.render());
    

with open(os.path.join(args.outdir, name + ".tsv"), 'w') as f:
    f.write("%s\n" % "\t".join(header[1:]));
    for peak in peaks:
