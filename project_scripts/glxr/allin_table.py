...
header = ["Chrom", "Start", "Stop", "Gene ID", "Gene symbol", "Distance ATG", "Distance to TSS", "Predicted Function", "Annotation"] + ["TC %s" % x for x in sample_names] + ["AC %s" % x for x in sample_names] + ["AS Gene ID", "AS Gene symbol", "AS Distance ATG", "AS Distance to TSS", "AS Predicted Function", "AS Annotation"]
...
def output_interval(interval, antisense):
    a = [interval.chrom, str(interval.start), str(interval.stop), interval.attrs['cg'],  interval.attrs['genesymbol'], interval.attrs['atg'], interval.attrs['tss'], interval.attrs['function'], interval.attrs['annotation']] + interval.attrs['topcoverage'].split(",") + interval.attrs['area_coverage'].split(",") 
    if(antisense):
...
for interval, _ in sorted(passed_intervals, key = lambda x: x[1], reverse = True):
    ...
    output_interval(interval, antisense);
