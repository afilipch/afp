...
with open(args.path) as f:
    ...
    for l in f:
        a = l.strip().split("\t")
        if(a[9] != 'None' and a[21] != 'None' and float(a[6]) < 350):
            data.append(( a[4], float(a[9]), float(a[21]) ))


fontsize=24
linewidth = 5
xvals = np.array([log(x[1], 2) for x in data])
yvals = np.array([x[2] for x in data])

fig, ax = plt.subplots(figsize=(16,9))
plt.tight_layout(rect=[0.1, 0.1, 0.95, 0.95])

ax.set_xlabel('Log2(Peak Intensity) [T=30m]', fontsize=fontsize)
ax.set_ylabel('Log2(Gene expression fold change) [T=30m]', fontsize=fontsize)    
