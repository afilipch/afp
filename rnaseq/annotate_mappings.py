...
def fill_gene_types(mappings, annotation, gene_types, multi):
    offset = len(mappings[0].fields)
    curname = '';
    curints = [];
    for interval in mappings.intersect(annotation, s = True, wao = True, f = 0.5):
...
fill_gene_types(mappings, annotation, gene_types, False);

if(args.multi):
    mappings = BedTool(args.multi)
    fill_gene_types(mappings, annotation, gene_types, True);
